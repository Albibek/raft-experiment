...
pub struct Actions {
    ...
    pub peer_messages: Vec<(ServerId, Arc<Builder<HeapAllocator>>)>,
    pub peer_endpoints: BTreeMap<ServerId, UnboundedSender<RaftEgress>>,
    /// Messages to be send to clients.
    pub client_messages: Vec<(ClientId, Arc<Builder<HeapAllocator>>)>,
    ...
    M: StateMachine,
{
    ...
    fn append_entries_request(
        ...
        if leader_term < current_term {
            return Ok(ConsensusResponse::StaleTerm(current_term));
        }
    ...
    fn request_vote_request(
                   ...
                   candidate_log_index < self.latest_log_index()
        {
            ...
            match self.log.voted_for().unwrap() { // TODO deal with unwrap
                ...
                _ => Ok(RequestVoteResponse::AlreadyVoted(new_local_term)),
            }
        };
        message
    }
    ...
    fn heartbeat_timeout(&mut self, peer: ServerId, actions: &mut Actions) {
        ...
        actions.peer_messages.push((peer, Arc::new(message)));
    }
    ...
    fn transition_to_follower(&mut self, term: Term, leader: ServerId) {
        ...
        // FIXME     actions.clear_timeouts = true;
        //     actions.clear_peer_messages = true;
        // FIXME actions.timeouts.push(ConsensusTimeout::Election);
    }
...
//) -> Vec<(ClientId, Arc<Builder<HeapAllocator>>)> {
//let mut queue: VecDeque<(ServerId, ServerId, Arc<Builder<HeapAllocator>>)> = VecDeque::new();
